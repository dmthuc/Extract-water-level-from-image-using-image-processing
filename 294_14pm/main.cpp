...
int main(int argc, char ** argv)
{
	...
	{
		std::cerr << "err: input image is not valid";
		return 0;
	}
	...
	/**classify*/
	vector<int> symbols_result;
	ANN nn = ANN(MODEL_PATH, MODEL_NAME);
	for (int i = 0; i < cropped_images.size(); i++)
	{
		int result = nn.classify(cropped_images[i], program_mode);
		symbols_result.push_back(result);	
	}

	for (int i = 0; i < symbols_result.size(); i++)
	{

		if (symbols_result[i] == 2 ||symbols_result[i] == 1 || symbols_result[i] == 9 || symbols_result[i] == 0)
		{

		}
		else
		{
			std::cout << symbols_result[i] << " ";
		}
